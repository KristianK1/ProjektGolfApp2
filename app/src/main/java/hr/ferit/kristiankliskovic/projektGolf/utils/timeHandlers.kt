...
fun CalendartoMapMarkerString(cal: Calendar): String{
    val ispis: String
    ispis = "${cal.get(Calendar.HOUR_OF_DAY)}:${cal.get(Calendar.MINUTE)}:${cal.get(Calendar.SECOND)} ${cal.get(Calendar.DAY_OF_MONTH)}.${cal.get(Calendar.MONTH) + 1}.${cal.get(Calendar.YEAR)}"
    return ispis
}

fun addTimeToCalendar(calendarIn: Calendar,years: Int, months: Int, days: Int, hours: Int, mins: Int, seconds: Int): Calendar {
